...
M.on_attach = function(client, bufnr)
  ...
  local opts = {
    ...
    buffer = bufnr, -- restrict the keymap to the local buffer
  }
  ...
  keymap("n", "<leader>gt", "<cmd>FzfLua lsp_typedefs<CR>", opts) -- Jump to the type defintion of the symbol under the cursor
  keymap("n", "<leader>ds", "<cmd>FzfLua lsp_document_symbols<CR>", opts) -- List all symbols (functions, classes, etc.) in the current file
  keymap("n", "<leader>ws", "<cmd>FzfLua lsp_workspace_symbols<CR>", opts) -- Search for any symbol across the entire workspace
